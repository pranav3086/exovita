...
<div className={`absolute top-32 right-10 flex flex-col gap-8 z-40 transition-all duration-1000 ${simState !== "idle" ? "translate-x-[120%] opacity-0" : "translate-x-0 opacity-100"}`}>
  <div className="w-80 bg-slate-900/30 backdrop-blur-xl border border-white/10 rounded-[2rem] p-8 shadow-2xl">
    ...
    {/* Updated action path to point to the root-level public file */}
    <form action="/exoplanet_ai_system.html" method="get" className="relative mb-6">
       <input 
         type="text" 
         name="search" 
         placeholder="Search NASA Archives..." 
         className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-[10px] focus:outline-none focus:border-cyan-400 transition-all" 
       />
    </form>
    <div className="p-5 rounded-2xl bg-cyan-400/5 border border-cyan-400/10 text-[10px] text-white/50 leading-relaxed font-mono italic">
      "Cross-referencing real-time telemetry with interstellar database"
    </div>
    {/* Updated href to use an absolute path */}
    <a 
      href="/exoplanet_ai_system.html" 
      className="mt-8 flex items-center justify-center gap-3 w-full py-4 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] transition-all group"
    >
